/**
 * Comprehensive test cases from parse-address library for compatibility
 * These tests ensure we handle all the same address formats as the original parse-address
 */

import { describe, test, expect } from 'vitest';
import { parseLocation, parseIntersection } from '../parser';
...
describe('Parse-Address Compatibility Tests', () => {
  describe('Basic Address Parsing', () => {
    ...
    Object.entries(poBoxTests).forEach(([input, expected]) => {
      test(`should parse PO Box: ${input}`, () => {
        const result = parseLocation(input);
        expect(result).toMatchObject(expected);
      });
    });
  ...
  describe('Intersection Parsing', () => {
    ...
    Object.entries(intersectionTests).forEach(([input, expected]) => {
      test(`should parse intersection: ${input}`, () => {
        const result = parseIntersection(input);
        expect(result).toMatchObject(expected);
      });
    });
  });
  ...
  describe('Special Cases', () => {
    test('should handle case insensitive addresses', () => {
      const result = parseLocation('123 main street new york ny');
    ...
    test('should handle various directional formats', () => {
      const testCases = [
        '100 South St, Philadelphia, PA',
        '100 S.E. Washington Ave, Minneapolis, MN',
        '233 S Wacker Dr 60606-6306'
    ...
    test('should handle grid-based addresses', () => {
      const result = parseLocation('N95W18855 Jay Dr Menomonee Falls WI 53051');
    ...
    test('should handle unusual street types', () => {
      const unusualTypes = [
        '2672 Industrial Row Troy, MI 48084',
        '415 Van Wyck Mews Norfolk VA 23517',
        '22 Cumbres Pass Santa Fe New Mexico 87508',
      ...
      unusualTypes.forEach(address => {
        const result = parseLocation(address);
        expect(result).toBeTruthy();
      });
    });

    test('should normalize street types correctly', () => {
      const result = parseLocation('123 Main Street, New York, NY');
