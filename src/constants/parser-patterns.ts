...
const PO_BOX_PATTERNS = {
  ...
  US_PO_BOX: /^(?:p\.?\s*o\.?\s*box|post\s*office\s*box|pobox|po\s*box)\s*(\d+)\s*,?\s*([^,]+?)\s*,?\s*([A-Za-z]{2})\s*(\d{5}(?:[-\s]?\d{4})?)?\s*$/i,
  
  // Station/Succursale/RPO/RR pattern for Canadian addresses
  STATION_PATTERN: /^(station|succ(?:\.|ursale)?|rpo|rr|r\.r\.)\s+([A-Za-z0-9]+(?:\s*[A-Za-z0-9-]+)*?)(?=\s+[A-Z][a-z]|\s*,|$)/i,
...
const INTERSECTION_PATTERNS = {
  ...
  STREET_SIMPLE: (directionalPattern: string) => 
    new RegExp(`^(?:(${directionalPattern})\\s+)?(.+)$`, 'i')
} as const;
...
const COMMON_PARSER_PATTERNS = {
  ...
  CITY_STATE_PATTERN: (stateAbbrevPattern: string) => new RegExp(`^(.+?)\\s+(${stateAbbrevPattern.slice(2, -2)})\\s*$`, 'i'),
  
  // Postal code at end with international pattern
  POSTAL_AT_END: (postalPattern: string) => new RegExp(postalPattern + '$', 'i')
} as const;

export { PO_BOX_PATTERNS, INTERSECTION_PATTERNS, COMMON_PARSER_PATTERNS };